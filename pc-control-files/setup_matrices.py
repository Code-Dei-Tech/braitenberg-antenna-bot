# Apriltag robot coordinate transformation 
# Hans Verdolaga
# MSc Mechatronics 2023
# MC-F23 Thesis

# Apriltag video coordinate detection and transformation
# Hans Verdolaga
# MSc Mechatronics 2023
# MC-F23 Thesis
...
# bottomLeftCoordList = cfg.realBottomLeft
# bottomRightCoordList = cfg.realBottomRight
# topRightCoordList = cfg.realTopRight
# topLeftCoordList = cfg.realTopLeft



# Transform perspective image to topdown view
# def transform_perspective(img, pts1, pts2):
...
dictList = [blImgCoordDict, brImgCoordDict, trImgCoordDict, tlImgCoordDict]
...
def main(args, matType, outputType):
    ...
    for i in range(cfg.refFrames):
        ...
        ret, frame = cap.read()
        if ret == True:
            # cv.imwrite('Implementation\\Host_24-04-23\\image_test\\image_setup.jpg', frame)
    ...
    for dict in dictList:
        ...
        # print(dict['per_center'])
        # dict['per_center'] = np.multiply(np.mean(dict['list_center'], axis=0), cfg.scaleFactor)
        pts1List.append(dict['per_center'])

    pts1 = np.float32(pts1List)

    # Destination points
    if outputType == 0: pts2 = np.float32([cfg.realBottomLeft, cfg.realBottomRight, 
                                        cfg.realTopRight, cfg.realTopLeft])
    else: pts2 = np.float32([cfg.topViewBottomLeft, cfg.topViewBottomRight, 
                            cfg.topViewTopRight, cfg.topViewTopLeft])

    # print('Image coordinates of Apriltag borders in pixels')
    # print(pts1)
    # if outputType == 0:
    #     print('\nReal coordinates of Apriltag borders in mm')
    # else: 
    #     print('\nViewport coordinates of Apriltag borders in pixels')
    # print(pts2)

    # Get transformation matrices
    transMat = coordFunc.getPerspectiveTransformMatrix(pts1, pts2)
    ...
    if matType == 0: configTag = 'source'
    else: configTag = 'robot'

    if outputType == 0: configMatSuffix = 'ToReal'
    else: configMatSuffix = 'ToView'
    
    matSearch = configTag + configMatSuffix + 'Matrix'
    matReplace = f'{matSearch} = np.array({transMat.tolist()})'

    for line in filedata.splitlines():
        if matSearch in line:
            filedata = filedata.replace(line, matReplace)
